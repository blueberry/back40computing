...
of-boundFULL_TILEELEMENTS) || (tile_element < guarded_e
		 ...
		 */
		template <typename T>
		static __device__ __forceinline__ void AlignedScatterSmemStorage 	&cta_smem_storage,
			T 				*buffer,
			T 				*d_out,
			SizeT 			valid_elements)
		{
			...
			{IGITS) {
				...
				int my_exclus	= cta_smem_storage.digit_prefixes[my_digit];
			...
			// Next scatter pass
			Iterate<COUNT + 1, MAX>::AlignedScacta_smem_storage, buffer, d_out, valid_elements);
		}
	};
	...
	__device__ __forceinline__ void LoadKeys(
		...
		const SizeT &gcta_uarded_elements,SizeT 			bin_prefix,
		KeyType 		*d_keys_in,
		KeyType 		*d_keys_out,
		ValueType 		*d_values_in,
		ValueType 		*d_values_out,
...
Shar// Gather values from shared
	...
	{n[0][16 + threadIdx.x] = bin_inctemplate <bool FULL_TILE>nclusive;
			PackedCounter bin_exclusive = smem_storagectarpscan[0][1const SizeT &.x - 1];
		...
		GatherScatterValues<FULL_TILE>(values, digit_offsets, ranks, cta_offset, guarded_elementsked(*thipublic:im.x * threadIdx.x) + blockIdx.x;
