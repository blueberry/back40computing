...
*/
	...
	struct SoaSumOp
	{
			...
			WarpscanT warpscan1)
		{
			...
			warpscan_soa.Get(input_partial, 16 + threadIdx.x - 0);

			warpscan_soa.Get(current_partial, 16 + threadIdx.x - 1);
			inclusive_partial = scan_op(current_partial, input_partial);
			warpscan_soa.Set(inclusive_partial, 16 + threadIdx.x);

			warpscan_soa.Get(current_partial, 16 + threadIdx.x - 2);
			inclusive_partial = scan_op(current_partial, inclusive_partial);
			warpscan_soa.Set(inclusive_partial, 16 + threadIdx.x);

			warpscan_soa.Get(current_partial, 16 + threadIdx.x - 4);
			inclusive_partial = scan_op(current_partial, inclusive_partial);
			warpscan_soa.Set(inclusive_partial, 16 + threadIdx.x);

			warpscan_soa.Get(current_partial, 16 + threadIdx.x - 8);
			inclusive_partial = scan_op(current_partial, inclusive_partial);
			warpscan_soa.Set(inclusive_partial, 16 + threadIdx.x);

			warpscan_soa.Get(current_partial, 16 + threadIdx.x - 16);
			inclusive_partial = scan_op(current_partial, inclusive_partial);
			warpscan_soa.Set(inclusive_partial, 16 + threadIdx.x);

			warpscan_soa.Get(total, 16 + 31);
	...
	__device__ __forceinline__ void ScanTile(Cta *cta, const int CURRENT_BIT, const bool shift_bytes)
	{
		...
		IterateTileElements<0, 0>::DecodeKeys(cta, dispatch, CURRENT_BIT);
...
*/
			...
			int offset = (threadIdx.x < RAKING_THREADS / 2) ?
				0 :
				(B40C_WARP_THREADS(CUDA_ARCH) * 3 / 2) - 16;

			// Unpack byte-packed partial sum into short-packed partial sums
			cta->smem_storage.warpscan[0][16 + threadIdx.x + offset] 		= util::PRMT(partial_bytes, 0, 0x4240);
			cta->smem_storage.warpscan[0][16 + threadIdx.x + offset + 16] 	= util::PRMT(partial_bytes, 0, 0x4341);
...
*/
			...
			cta->smem_storage.warpscan[0][16 + threadIdx.x] = exclusive_partial.t0;
			cta->smem_storage.warpscan[1][16 + threadIdx.x] = exclusive_partial.t1;
			...
			int b = cta->smem_storage.warpscan[0][16 + other];								// 1,3
			int c = exclusive_partial.t1;													// 4,6
			int d = cta->smem_storage.warpscan[1][16 + other];								// 5,7
		...
		IterateTileElements<0, 0>::ComputeRanks(cta, dispatch, shift_bytes);
	}
