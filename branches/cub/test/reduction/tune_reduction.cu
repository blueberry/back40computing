...
using namespace b40c::reduction;
...
{
	typedef cudaError_t (*MapTextures)(void *, int);
...
struct UpsweepTuning
{
	...
	template <
		...
		typename BaseKernelPolicy =
			reduction::KernelPolicy <
				...
				(util::io::ld::CacheModifier) util::Access<ParamList, READ_MODIFIER>::VALUE,				// READ_MODIFIER,
	...
	struct KernelPolicy : BaseKernelPolicy
	{
		...
		{
			char buffer[4096];
			sprintf(buffer, "%s, %s, %s, %s, %s, %s, %d, %d, %d, %d",
				CacheModifierToString(KernelPolicy::READ_MODIFIER),
				CacheModifierToString(KernelPolicy::WRITE_MODIFIER),
		...
		static cudaError_t MapTextures(void *d_src, int num_elements)
		{
...
struct Enactor : public util::EnactorBase
{
		...
		SpineLaunchDetails spine_details)
	{
		...
		do {
			...
			if (work.grid_size == 1) {
				...
				// Bind input texture
				if (retval = util::B40CPerror(spine_details.first.map_textures((void *) d_src, work.num_elements),
				...
				// Bind input texture
				if (retval = util::B40CPerror(upsweep_details.first.map_textures((void *) d_src, work.num_elements),
					"Enactor cudaBindTexture InputTex failed", __FILE__, __LINE__)) break;
	...
	void IterateConfigSpace()
	{
			...
			upsweep_config_itr++)
		{
			...
			typename SpineMap::iterator spine_config_itr = spine_configs.begin();
/*
