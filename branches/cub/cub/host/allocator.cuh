	...
	cudaError_t HostReset()
	{
		...
		do {
			...
			if (retval = util::B40CPerror(cudaSetDevice(gpu),
				"cudaSetDevice failed: ", __FILE__, __LINE__)) break;
#endif
			if (d_spine) {
				// Deallocate
				if (retval = util::B40CPerror(cudaFree(d_spine),
					"cudaFree d_spine failed: ", __FILE__, __LINE__)) break;
				d_spine = NULL;
			}
			if (h_spine) {
				// Deallocate
				if (retval = util::B40CPerror(cudaFreeHost((void *) h_spine),
					"cudaFreeHost h_spine failed", __FILE__, __LINE__)) break;
