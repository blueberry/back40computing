...
#include <back40/radix_sort/sort_utils.cuh>
#include <back40/radix_sort/downsweep/kernel_policy.cuh>
#include <back40/radix_sort/downsweep/tex_ref.cuh>
#include <back40/radix_sort/spine/tex_ref.cuh>
...
template <
	...
	typename ValueType>
struct Cta :
	cub::CtaProgress<				// Progress-management base class
		SizeT,
		KernelPolicy::TILE_ELEMENTS>
{
	...
	typedef CtaProgress<
		SizeT,
		KernelPolicy::TILE_ELEMENTS> CtaProgress;
	...
	SmemStorage 				&smem_storage;		// Shared storage for this CTA
	int 						warp_id;
	...
	SizeT 						global_digit_base;	// The global scatter base offset for each digit (valid in the first RADIX_DIGITS threads)
	...
	struct Iterate
	{
			...
			T items[THREAD_ELEMENTS])
		{
			int offset = (BANK_PADDING) ?
				cub::SHR_ADD(tile.ranks[COUNT], LOG_MEM_BANKS, tile.ranks[COUNT]) :
				tile.ranks[COUNT];
		...
		static __device__ __forceinline__ void DecodeBinOffsets(Cta &cta, Tile &tile)
		{
			...
			unsigned int byte_offset = Extract<
				...
				CURRENT_BIT,
				RADIX_BITS,
				cub::Log2<sizeof(SizeT)>::VALUE>::SuperBFE(
					tile.keys[COUNT]);
			...
			const SizeT &guarded_elements)
		{
			...
			if ((guarded_elements >= TILE_ELEMENTS) || (tile_element < guarded_elements))
			{
				...
				cub::Store<STORE_MODIFIER>::St(scatter, items[COUNT]);
			}
			...
			if (my_digit < RADIX_DIGITS)
			{
				...
				while (my_aligned_offset < my_digit_count)
				{
					if ((my_aligned_offset >= 0) && (my_exclusive_scan + my_aligned_offset < valid_elements))
					{
						...
						cub::Store<STORE_MODIFIER>(d_out + my_carry + my_aligned_offset, datum);
					}
		...
		{
			// Load tile of values
			cta.LoadValues(cta_offset, guarded_elements, tile);
	...
	struct Iterate<MAX, MAX>
	{
		...
		// TruckValues
		static __device__ __forceinline__ void TruckValues(SizeT cta_offset, const SizeT &guarded_elements, Cta &cta, Tile &tile) {}
	};
		...
		Tile &tile)
	{
		...
		{
			// Unguarded loads
			cub::CtaLoad<THREADS, LOAD_MODIFIER>::LoadUnguarded(
				tile.keys,
				(Cta::FLOP_TURN) ?
		...
		else
		{
			...
			#pragma unroll
			for (int ELEMENT = 0; ELEMENT < THREAD_ELEMENTS; ELEMENT++)
			{
				tile.keys[ELEMENT] = KeyType(-1);
			}

			// Guarded loads
			cub::CtaLoad<THREADS, LOAD_MODIFIER>::LoadGuarded(
				tile.keys,
				d_in_keys,
				cta_offset,
				guarded_elements);
	...
	{
		// Upsweep reduce
		PackedCounter raking_partial = cub::Reduce<RAKING_SEGMENT>(raking_segment);
		...
		cub::ExclusiveScan<RAKING_SEGMENT>(raking_segment, exclusive_partial);
	}
		...
		const SizeT &guarded_elements = TILE_ELEMENTS)
	{
		...
		// Load tile of keys
		LoadKeys(cta_offset, guarded_elements, tile);
		...
		Iterate<0, !KEYS_ONLY>::TruckValues(cta_offset, guarded_elements, *this, tile);
	}
