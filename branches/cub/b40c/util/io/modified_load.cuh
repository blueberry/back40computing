...
namespace ld {
	...
	enum CacheModifier {
		...
		ca,					// cache all
		cs, 				// cache streaming
		tex,				// texture (defaults to NONE if no tex reference is provided)
...
#define CacheModifierToString(modifier)	(	((int) modifier == b40c::util::io::ld::NONE) ? 		"util::io::ld::NONE" :	\
											...
											((int) modifier == b40c::util::io::ld::tex) ? 		"util::io::ld::tex" :		\
...
struct ModifiedLoad
{
	...
	/**
	 * Specialization for tex modifier (tex loads handled elsewhere)
	 */
	template <>
	template <typename T>
