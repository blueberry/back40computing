...
struct CsrProblem
{
		...
		double queue_sizing)			// Size scaling factor for work queue allocation (e.g., 1.0 creates n-element and m-element vertex and edge frontiers, respectively).  0.0 is unspecified.
	{
		...
		for (int gpu = 0; gpu < num_gpus; gpu++) {
			...
			memset_grid_size = CUB_MIN(memset_grid_size_max, (graph_slices[gpu]->nodes + memset_block_size - 1) / memset_block_size);
			...
			memset_grid_size = CUB_MIN(memset_grid_size_max, (visited_mask_elements + memset_block_size - 1) / memset_block_size);
