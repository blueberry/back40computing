...
struct KernelPolicy : TuningPolicy
{
	enum {		// N.B.: We use an enum type here b/c of a NVCC-win compiler bug involving ternary expressions in static-const fields
		...
		LOG_COMPOSITES_PER_LANE_PER_THREAD 	= LOG_COMPOSITES_PER_LANE - CUB_LOG_WARP_THREADS(TuningPolicy::CUDA_ARCH),					// Number of partials per thread to aggregate
		COMPOSITES_PER_LANE_PER_THREAD 		= 1 << LOG_COMPOSITES_PER_LANE_PER_THREAD,
	
		AGGREGATED_ROWS						= BINS,
		AGGREGATED_PARTIALS_PER_ROW 		= CUB_WARP_THREADS(TuningPolicy::CUDA_ARCH),
	...
	enum {
		THREAD_OCCUPANCY					= CUB_SM_THREADS(TuningPolicy::CUDA_ARCH) >> TuningPolicy::LOG_THREADS,
		SMEM_OCCUPANCY						= CUB_SMEM_BYTES(TuningPolicy::CUDA_ARCH) / sizeof(SmemStorage),
		MAX_CTA_OCCUPANCY					= CUB_MIN(CUB_SM_CTAS(TuningPolicy::CUDA_ARCH), CUB_MIN(THREAD_OCCUPANCY, SMEM_OCCUPANCY)),
