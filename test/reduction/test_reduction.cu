...
void TestReduction(size_t num_elements)
{
	...
	do {
		...
		printf("\nLARGE config:\t");
		double large = TimedReduction<T, BinaryOp, Identity, reduction::LARGE>(h_data, h_reference, num_elements);

		printf("\nSMALL config:\t");
		double small = TimedReduction<T, BinaryOp, Identity, reduction::SMALL>(h_data, h_reference, num_elements);
