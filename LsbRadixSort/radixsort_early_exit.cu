...
protected:
	...
	struct DefaultSortingConfig
	{
		typedef typename MultiCtaLsbSortEnactor<KeyType, ConvertedKeyType, ValueType>::template MultiCtaConfig<
			...
			(SM_ARCH >= 200) ? 					10 :	// 1024 elements on GF100+
			...
			// EARLY_EXIT								// Default (terminate downsweep if homogeneous digits)
			true,
	...
	//---------------------------------------------------------------------
	// Small-Problem Granularity Parameterization
	//---------------------------------------------------------------------
     ...
     */
	template <typename SortingConfig>
    cudaError_t PostSort(typename SortingConfig::Storage &problem_storage, int passes)
    {
	...
	struct Architecture
	{
		...
		{
// mooch
//			typedef typename SmallSortingConfig<SM_ARCH, Storage>::Config SortingConfig;
...
public:
	...
	cudaError_t EnactSort(typename SortingConfig::Storage &problem_storage) 
	{
	    ...
	    PostSort<SortingConfig>(problem_storage, NUM_PASSES);
