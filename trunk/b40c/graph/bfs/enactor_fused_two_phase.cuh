...
#include <b40c/graph/bfs/fused_two_phase/kernel_policy.cuh>
#include <b40c/graph/bfs/fused_two_phase/kernel.cuh>
 ...
 */
class EnactorFusedTwoPhase : public EnactorBase
{
...
public: 	
	 ...
	 * Constructor
	 */
	EnactorFusedTwoPhase(bool DEBUG = false) :
		EnactorBase(DEBUG),
		iteration(NULL),
		d_iteration(NULL)
	{}
	 ...
	 * Destructor
	 */
	virtual ~EnactorFusedTwoPhase()
	{
		if (iteration) util::B40CPerror(cudaFreeHost((void *) iteration), "EnactorFusedTwoPhase cudaFreeHost iteration failed", __FILE__, __LINE__);
	}
		...
		do {
			...
			if (!iteration) {
				...
				if (retval = util::B40CPerror(cudaHostAlloc((void **)&iteration, sizeof(long long) * 1, flags),
					"EnactorFusedTwoPhase cudaHostAlloc iteration failed", __FILE__, __LINE__)) break;

				// Map into GPU space
				if (retval = util::B40CPerror(cudaHostGetDevicePointer((void **)&d_iteration, (void *) iteration, 0),
					"EnactorFusedTwoPhase cudaHostGetDevicePointer iteration failed", __FILE__, __LINE__)) break;
			}
		...
		int 							max_grid_size = 0)
	{
		...
		do {
			...
			if (retval = util::B40CPerror(cudaBindTexture(
				...
				"EnactorFusedTwoPhase cudaBindTexture row_offset_tex_ref failed", __FILE__, __LINE__)) break;

			// Initiate single-grid kernel
			fused_two_phase::Kernel<KernelPolicy>
