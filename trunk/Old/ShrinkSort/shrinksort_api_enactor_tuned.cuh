...
#include "shrinksort_common.cuh"
#include "shrinksort_api_enactor.cuh"
#include "shrinksort_api_granularity.cuh"
#include "shrinksort_granularity_tuned_large.cuh"
#include "shrinksort_granularity_tuned_small.cuh"
...
template <TunedGranularityEnum GRANULARITY_ENUM, int CUDA_ARCH, typename KeyType, typename ValueType, typename SizeT>
...
__global__ void TunedDownsweepKernel(int * __restrict d_selectors, SizeT * __restrict d_spine, ConvertedKeyType * __restrict d_keys0, ConvertedKeyType * __restrict d_keys1, ValueType * __restrict d_values0, ValueType * __restrict d_values1, CtaWorkDistribution<SizeT> work_decomposition);
 ...
 */
class ShrinkSortEnactorTuned :
	public ShrinkSortEnactor<ShrinkSortEnactorTuned>,
	public Architecture<__B40C_CUDA_ARCH__, ShrinkSortEnactorTuned>
{

protected:

	// Typedefs for base classes
	typedef ShrinkSortEnactor<ShrinkSortEnactorTuned> 					BaseEnactorType;
	typedef Architecture<__B40C_CUDA_ARCH__, ShrinkSortEnactorTuned> 	BaseArchType;
	...
	cudaError_t DigitPlacePass(Decomposition &work)
	{
		...
		do {
			...
			if (this->DEBUG && (retval = B40CPerror(cudaThreadSynchronize(),
				"ShrinkSortEnactorTuned:: TunedUpsweepKernel failed ", __FILE__, __LINE__))) break;
			...
			TunedSpineScanKernel<KeyType, ValueType, SizeT, SortingConfig::GRANULARITY_ENUM>
				<<<grid_size[1], threads[1], dynamic_smem[1]>>>(
					...
					work.spine_elements);
			if (this->DEBUG && (retval = B40CPerror(cudaThreadSynchronize(),
				"ShrinkSortEnactorTuned:: TunedSpineScanKernel failed ", __FILE__, __LINE__))) break;
	...
	cudaError_t Enact(Storage &problem_storage, Detail &detail)
	{
		...
		typedef TunedGranularity<
			...
			typename Storage::SizeT> SortingConfig;

		// Enact sort using that type
		return ((BaseEnactorType *) this)->template Enact<
				Storage, 
				SortingConfig, 
				Detail::START_BIT, 
				Detail::NUM_BITS>(
...
public:
	 ...
	 * Constructor.
	 */
	ShrinkSortEnactorTuned() : BaseEnactorType::ShrinkSortEnactor() {}
