...
#include "shrinksort_kernel_upsweep.cuh"
#include "shrinksort_kernel_spine.cuh"
#include "shrinksort_kernel_downsweep.cuh"

namespace b40c {

using namespace shrink_sort;


/**
 * Forward declaration of an internal utility type for managing the state for a
 * specific sorting operation.  TODO: Class can be moved inside ShrinkSortEnactor
...
class ShrinkSortEnactor : public EnactorBase
{
protected:
	...
	typedef typename DispatchType<ShrinkSortEnactor, DerivedEnactorType>::Type Dispatch;
		...
		do {
			...
			if (problem_storage.d_keys[0] == NULL) {
				if (retval = B40CPerror(cudaMalloc((void**) &problem_storage.d_keys[0], problem_storage.num_elements * sizeof(KeyType)),
					"ShrinkSortEnactor cudaMalloc problem_storage.d_keys[0] failed", __FILE__, __LINE__)) break;
			}
			if (problem_storage.d_keys[1] == NULL) {
				if (retval = B40CPerror(cudaMalloc((void**) &problem_storage.d_keys[1], problem_storage.num_elements * sizeof(KeyType)),
					"ShrinkSortEnactor cudaMalloc problem_storage.d_keys[1] failed", __FILE__, __LINE__)) break;
			}
			if (!IsKeysOnly<ValueType>()) {
				if (problem_storage.d_values[0] == NULL) {
					if (retval = B40CPerror(cudaMalloc((void**) &problem_storage.d_values[0], problem_storage.num_elements * sizeof(ValueType)),
						"ShrinkSortEnactor cudaMalloc problem_storage.d_values[0] failed", __FILE__, __LINE__)) break;
				}
			...
			if (problem_spine_bytes > spine_bytes) {
				if (d_spine) {
					if (retval = B40CPerror(cudaFree(d_spine),
						"ShrinkSortEnactor cudaFree d_spine failed", __FILE__, __LINE__)) break;
				}

				spine_bytes = problem_spine_bytes;

				if (retval = B40CPerror(cudaMalloc((void**) &d_spine, spine_bytes),
					"ShrinkSortEnactor cudaMalloc d_spine failed", __FILE__, __LINE__)) break;
			}
		} while (0);
		...
		if (!SortingConfig::Upsweep::EARLY_EXIT) {
			...
			do {
				...
				if (retval = B40CPerror(cudaMemcpy(&problem_storage.selector, &d_selectors[passes & 0x1], sizeof(int), cudaMemcpyDeviceToHost),
					"ShrinkSortEnactor cudaMemcpy d_selector failed", __FILE__, __LINE__)) break;
	...
	{
		using namespace shrink_sort::upsweep;
		using namespace shrink_sort::spine_scan;
		using namespace shrink_sort::downsweep;
		...
		do {
			...
			if (SortingConfig::UNIFORM_SMEM_ALLOCATION) {
				...
				if (retval = B40CPerror(cudaFuncGetAttributes(&upsweep_kernel_attrs, UpsweepKernel<UpsweepKernelConfigType>),
					"ShrinkSortEnactor cudaFuncGetAttributes upsweep_kernel_attrs failed", __FILE__, __LINE__)) break;
				if (retval = B40CPerror(cudaFuncGetAttributes(&spine_scan_kernel_attrs, SpineScanKernel<SpineScanKernelConfigType>),
					"ShrinkSortEnactor cudaFuncGetAttributes spine_scan_kernel_attrs failed", __FILE__, __LINE__)) break;
				if (retval = B40CPerror(cudaFuncGetAttributes(&downsweep_attrs, DownsweepKernel<DownsweepKernelConfigType>),
					"ShrinkSortEnactor cudaFuncGetAttributes downsweep_attrs failed", __FILE__, __LINE__)) break;
			...
			UpsweepKernel<UpsweepKernelConfigType>
				<<<grid_size[0], threads[0], dynamic_smem[0]>>>(
					...
					work.problem_storage->d_keep,
					work);
			if (DEBUG && (retval = B40CPerror(cudaThreadSynchronize(),
				"ShrinkSortEnactor:: UpsweepKernel failed ", __FILE__, __LINE__))) break;
			...
			SpineScanKernel<SpineScanKernelConfigType>
				<<<grid_size[1], threads[1], dynamic_smem[1]>>>(
					...
					work.spine_elements);
			if (DEBUG && (retval = B40CPerror(cudaThreadSynchronize(),
				"ShrinkSortEnactor:: SpineScanKernel failed ", __FILE__, __LINE__))) break;
			...
			DownsweepKernel<DownsweepKernelConfigType>
				<<<grid_size[2], threads[2], dynamic_smem[2]>>>(
					...
					work);
			if (DEBUG && (retval = B40CPerror(cudaThreadSynchronize(),
				"ShrinkSortEnactor:: DownsweepKernel failed ", __FILE__, __LINE__))) break;
...
public:
     ...
     * Destructor
     */
    virtual ~ShrinkSortEnactor()
    {
   		if (d_spine) {
   			B40CPerror(cudaFree(d_spine), "ShrinkSortEnactor cudaFree d_spine failed: ", __FILE__, __LINE__);
   		}
   		if (d_selectors) {
   			B40CPerror(cudaFree(d_selectors), "ShrinkSortEnactor cudaFree d_selectors failed: ", __FILE__, __LINE__);
   		}
    }
