...
struct SwapAndScatter
{
	...
	struct PermuteValues
	{
			...
			SizeT scatter_offsets[KernelConfig::TILE_ELEMENTS_PER_THREAD])
		{
			...
			LoadTile<
				...
				KernelConfig::CACHE_MODIFIER,								// Cache modifier (e.g., CA/CG/CS/NONE/etc.)
			...
			Scatter<ValueType, int, KernelConfig::TILE_ELEMENTS_PER_THREAD, KernelConfig::THREADS, NONE, true>::Invoke(
					value_exchange, linear_values, linear_ranks, guarded_elements);

			__syncthreads();

			// Gather values from smem (vec-1)
			LoadTile<ValueType, int, KernelConfig::LOG_TILE_ELEMENTS_PER_THREAD, 0, KernelConfig::THREADS, NONE, true>::Invoke(
				reinterpret_cast<ValueType (*)[1]>(values), value_exchange, 0, guarded_elements);

			// Scatter values to global digit partitions
			Scatter<ValueType, SizeT, KernelConfig::TILE_ELEMENTS_PER_THREAD, KernelConfig::THREADS, KernelConfig::CACHE_MODIFIER, UNGUARDED_IO>::Invoke(
		...
		int 		key_ranks[KernelConfig::CYCLES_PER_TILE][KernelConfig::LOADS_PER_CYCLE][KernelConfig::LOAD_VEC_SIZE])		// The CTA-scope rank of each key
	{
		...
		Scatter<KeyType, int, KernelConfig::TILE_ELEMENTS_PER_THREAD, KernelConfig::THREADS, NONE, true>::Invoke(
			key_exchange, linear_keys, linear_ranks, guarded_elements);

		__syncthreads();

		// Gather keys from smem (vec-1)
		LoadTile<KeyType, int, KernelConfig::LOG_TILE_ELEMENTS_PER_THREAD, 0, KernelConfig::THREADS, NONE, true>::Invoke(
		...
		Scatter<
			...
			KernelConfig::CACHE_MODIFIER,
			UNGUARDED_IO,
			KernelConfig::PostprocessTraits::Postprocess>::Invoke(d_out_keys, linear_keys, scatter_offsets, guarded_elements);
...
__device__ __forceinline__ void ProcessTile(
	...
	typename KernelConfig::ValueType 	* __restrict d_in_values,
	typename KernelConfig::KeyType 	* __restrict d_out_keys,
	typename KernelConfig::ValueType 	* __restrict d_out_values,
	int 						*exchange,								
	int							lanes_warpscan[KernelConfig::SCAN_LANES_PER_CYCLE][3][KernelConfig::Grid::RAKING_THREADS_PER_LANE],
	...
	const typename KernelConfig::SizeT 		&guarded_elements)
{
	...
	LoadTile<
		...
		KernelConfig::CACHE_MODIFIER,								// Cache modifier (e.g., CA/CG/CS/NONE/etc.)
		UNGUARDED_IO,										// Whether or not bounds-checking is to be done
		KernelConfig::PreprocessTraits::Preprocess>				// Assignment function to transform the loaded value (or provide default if out-of-bounds)
	::Invoke(
