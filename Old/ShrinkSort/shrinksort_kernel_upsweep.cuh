...
struct BucketTileKeys
{
	...
	static __device__ __forceinline__ void Invoke(
		...
		int *composite_column)
	{
		Iterate<0, Config::LOADS_PER_TILE, 0, Config::LOAD_VEC_SIZE>::Invoke(
			keys, keep, composite_column);
	} 
};
	...
	char 						*hash_counters)
{
	...
	// Load tile of keys
	KeyType keys[Config::LOADS_PER_TILE][Config::LOAD_VEC_SIZE];
	char keep[Config::LOADS_PER_TILE][Config::LOAD_VEC_SIZE];			// Whether or not the key is a guaranteed duplicate
...
//		keys, keep, hash_pool);
	...
	StoreTile<
		...
		Config::THREADS,
		Config::CACHE_MODIFIER,
		true>::Invoke(keep, d_out_keep, cta_offset, 0);

	// Bucket tile of keys
	BucketTileKeys<Config>::Invoke(
		keys,
		keep,
		composite_column);
}
	...
	char 						*hash_counters)
{
	...
	if (threadIdx.x < Config::RADIX_DIGITS) {
		...
		SizeT digit_count = SerialReduce<SizeT, Config::AGGREGATED_PARTIALS_PER_ROW>::Invoke(
